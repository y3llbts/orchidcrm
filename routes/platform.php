<?php
  ...
  use App\Orchid\Screens\Tasks\TasksEditScreen;
  use App\Orchid\Screens\Tasks\TasksListScreen;
...
// Example...
  ...
  Route::screen('organizations/{organization}/edit', OrganizationEditScreen::class)
    ...
    ->breadcrumbs(fn(Trail $trail, $organization) => $trail
      ->parent('platform.organizations.list')
  ...
  Route::screen('organizations/create', OrganizationEditScreen::class)
    ...
    ->breadcrumbs(fn(Trail $trail) => $trail
      ->parent('platform.organizations.list')
      ->push(__('Создание организации'), route('platform.organizations.create')));
  ...
  Route::screen('tasks/{task}/edit', TasksEditScreen::class)
    ->name('platform.tasks.edit')
    ->breadcrumbs(fn(Trail $trail, $task) => $trail
      ->parent('platform.tasks.list')
      ->push($task->name, route('platform.tasks.edit', $task)));

  Route::screen('tasks/create', TasksEditScreen::class)
    ->name('platform.tasks.create')
    ->breadcrumbs(fn(Trail $trail) => $trail
      ->parent('platform.tasks.list')
      ->push(__('Создание проекта'), route('platform.tasks.create')));

  Route::screen('tasks', TasksListScreen::class)
    ->name('platform.tasks.list')
    ->breadcrumbs(fn(Trail $trail) => $trail
      ->parent('platform.index')
      ->push(__('Задачи'), route('platform.tasks.list')));
